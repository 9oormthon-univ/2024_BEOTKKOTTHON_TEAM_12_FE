...
export const useFormDataStore = create<FormDataStore>((set) => ({
  ...
  actions: {
    ...
    receiveData: (data) =>
      set(() => ({ formData: data, showImages: data.product_image })),
    resetFormData: () =>
      set(() => ({ formData: initialFormData, showImages: "" })),
    // setShowImages: (urls) => set(() => ({ showImages: [...urls] })),
...
export const useShowImages = () =>
  useFormDataStore((state) => state.showImages);
export const useFormDataActions = () =>
  useFormDataStore((state) => state.actions);
